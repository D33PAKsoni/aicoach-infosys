...
const [showWarning, setShowWarning] = useState(false);
  ...
  const [aiResponse, setAiResponse] = useState("Hello! I am your AI Interviewer. Please introduce yourself.");
  const [behaviorLog, setBehaviorLog] = useState([]); // Array to store behavior during answer
  ...
  const processTurn = async (blob) => {
    ...
    try {
      ...
      const turnResult = {
        ...
        fillers: chatRes.data.filler_words ? chatRes.data.filler_words.join(", ") : "None",
  ...
  const speakAndListen = async (text) => {
    ...
    const speakRes = await axios.post("http://localhost:8000/speak", speakFormData, { responseType: 'blob' });
  ...
  if(metrics.behaviorMetric == "Composed"){
     behaviorColor = "white";
  } else if(metrics.behaviorMetric == "Stressed"){
     behaviorColor = "yellow";
  }
    else if(metrics.behaviorMetric == "Confident"){
     behaviorColor = "green";
  }
  else {
      behaviorColor = "red";
  ...
  return (
    // <div>Interview Page - Resume: {resume.name}, JD: {jd?.name || 'None'}, Score: {score?.match_percentage || 0}%</div>
    <div className="interview-conatiner">         
      ...
      <div className="interview-dash">
        ...
        <div className="ai-window">
          ...
          {isListening ? <p>Listening</p> : <p>Processing...</p>}
          {isListening ? <Mic color='green' /> : <span className='spinner'><Loader2 color='white' size={30} /></span>}
        </div>
        ...
        <div className="answer-metrics">
            ...
            <>
              <div className="wpm">
                <p>Words Per Minute: {lastMetrics.wpm}</p>
              </div>
              <div className="filler-words">
                <p>Filler Words: {lastMetrics.fillers}</p>
              </div>
              <div className="accuracy">
                <p>Answer Accuracy: {lastMetrics.accuracy}%</p>
              </div>
              <div className="confidence">
                <p>Confidence Level: {lastMetrics.commonBehavior}</p>
              </div>
            </>
          ) : (
        ...
        <div className="metrics-panel">
            <div className="metrics-grid">
              ...
              <div className="metric-card">
                <div className='metric-name'><UserCheck size={40} color={behaviorColor}/>
                <span>Behavior</span></div>
                <span className="metric-value">{metrics.faceDetected? metrics.behaviorMetric : "Face Not Detected"}</span>
              </div>

              <div className="metric-card">
                <div className='metric-name'><Video size={40} color={metrics.possibleCheating ? "red" : "white"}/>
